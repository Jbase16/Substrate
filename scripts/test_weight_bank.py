...
# ---------------------------------------------------------------------------
# Helper: build a minimal plan that admits specific tiers per op.
# ---------------------------------------------------------------------------
def build_test_plan(
    model_id: str,
    layers: list,
    ...
    tier_precisions: e.g. {0: '4bit', 1: '6bit', 2: 'fp16_eq'}
                     means tier 0 = 4-bit, tier 1 = 6-bit, tier 2 = fp16.

    Bypasses the real RDO solver; this is a test fixture, not a real plan.
    Builds the same PlanBundle shape so the bank's API contract holds.
    """
    ...
    # Build catalog + op bundles. We synthesize the minimum the IR
    # validator demands. Tier-1/2 deltas are 0 because we deliberately
    # set the same precision across tiers for this test (we want the
    # bank to be exercised, not the IR's monotonicity rules).
    # Sort tiers by precision quality (higher precision = higher tier).
    bits_order = {"2bit": 2, "3bit": 3, "4bit": 4, "6bit": 6, "fp16_eq": 16}
    ...
    for layer_id in range(len(layers)):
        for kind, params, op_kind_enum in [
            ...
            # Catalog: skeleton + N residuals.
            sk_id = f"{op_id}.skeleton"
            sk_bytes = int(params * 2 / 8)  # 2-bit equivalent
            ...
            # Build a tier ladder. Tier 0 = lowest bits, tier N = highest.
            tiers_sorted = sorted(
                tier_precisions.items(),
                key=lambda kv: bits_order[kv[1]],
            )
            tiers: list[ScheduledOp] = []
            prev_bytes = sk_bytes
            for new_tier_idx, (orig_tier, prec) in enumerate(tiers_sorted):
                bits = bits_order[prec]
                ...
                # Quality risk monotonically non-increasing across tiers
                # (higher tier = better quality = lower risk).
                risk = max(0.0, 0.1 / max(1, new_tier_idx + 1))
                ram_delta = (
                    0 if new_tier_idx == 0
                    else max(0, full_bytes - int(params * bits_order[tiers_sorted[0][1]] / 8))
                )
    ...
    # Compute peak/pool to satisfy IR invariants.
    peak_ram = (
        sum(meta.bytes_in_ram for meta in catalog.values() if meta.is_skeleton)
        + 2_000_000_000   # rough overhead
    )
    pool = 4_000_000_000
    budget = Budget(
    ...
    return PlanBundle(
        ...
        solver_notes=("test plan, not from real solver",),
    )


def build_op_tier_precisions(plan, tier_precisions: dict[int, str]) -> dict[tuple[str, int], str]:
    """
    Build the (op_id, tier_index) -> precision_string map the
    RAMWeightStore needs.

    tier_precisions came from the user's intent; we map them through the
    plan's actual tier ordering (sorted by quality ascending).
    """
    bits_order = {"2bit": 2, "3bit": 3, "4bit": 4, "6bit": 6, "fp16_eq": 16}
    sorted_precisions = sorted(tier_precisions.values(), key=lambda p: bits_order[p])
